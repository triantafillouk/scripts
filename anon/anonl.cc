...
// initialize anonymized directory
	...
	result=system(s);
	printf("initilize dir %s result=%d\n",anon_log_dir,result);
	int max_category=0;
	char **category_files = get_dir(log_dir,&max_category);
	// for each category!
	for(int category_i=0;category_i<max_category;category_i++) {
		int max_subtypes=0;
		sprintf(subtype_dir,"%s/%s/queue",log_dir,category_files[category_i]);
		char **subtypes = get_dir(subtype_dir,&max_subtypes);
		printf("# %s -> %s %d subtypes\n",category_files[category_i],subtype_dir,max_subtypes);
#if	1
...
//			printf(" :  %s\n",archive_dir);
			sprintf(anon_archive_dir,"%s/%s/queue/%s/archive",anon_log_dir,category_files[category_i],subtypes[subtype_i]);
...
//			printf(" -> %s\n",anon_archive_dir);
			...
			int l=0;
			for(int l=0;l<archives_num;l++) {
				anonymize_file(archive_dir,archives[l],anon_archive_dir,category_files[category_i],subtypes[subtype_i]);
			};
		};
		...
		sprintf(category_dir,"%s/%s",log_dir,category_files[category_i]);
		int max_cat_files=0;
