...
alist *new_list(int id1,const char *title)
{
 alist *li;
 static int id=0;
...
BTREE *new_btree(const char *name,int level);
BTNODE *add_btnode(BTREE *bt,const char *name);
int delete_btnode_named(BTREE *bt,char *name);
int delete_btree(BTREE *bt);
BTNODE *find_btnode(BTREE *bt,const char *name);
...
BTNODE * insert_bt_element(BTREE *bt,const char *name,int type,int index)
{
 BTNODE *node;
...
int btival(BTREE *bt, const char *name)
{
 BTNODE *btn;
...
BTNODE *set_btival(BTREE *bt, const char *name,int value)
{
 BTNODE *btn;
 btn = find_btnode(bt,name);
 if(btn) {
...
int set_btnival(BTNODE *btn,int val)
{
